...
public class OntologyFilterTest {
   ...
   * nodes returned may be in the original list or may be children of nodes in
   * the original list.
   *
   * @param roots
   *   roots of trees to be operated on
   * @param predicate
   *   test for whether to remove category
   *
   * @return a list of flattened nodes
   */
  public static List<TreeNode<OntologyNode>> flattenCategories(
    List<TreeNode<OntologyNode>> roots,
    final Predicate<OntologyNode> predicate) {
    ...
    final Predicate<OntologyNode> customPred = obj -> {
    ...
    return masterRoot.mapStructure(
      (StructureMapper<OntologyNode, TreeNode<OntologyNode>>) (obj, mappedChildren) -> {
  ...
  public void testOntologyUsage() {
    ...
    rootContent.put(KEY, new ListBuilder<>(VALUE).toList());
    ...
    final Predicate<OntologyNode> TEST_PREDICATE = obj -> {
      List<String> values = obj.get(KEY);
      return values != null && values.contains(VALUE);
    };

    // will remove leaf (non-category) nodes that don't pass the predicate's test
    TreeNode<OntologyNode> filteredOntology = Ontology.getFilteredOntology(
      masterOntology, TEST_PREDICATE, true);
