...
public class UnitTestHelper {
  ...
  private static final AtomicLong nextStepId = new AtomicLong(0L);
  ...
  public synchronized static User getGuest() throws WdkModelException {
    if (guest == null) {
      guest = getModel().getUserFactory()
        .createUnregistedUser(UnregisteredUserType.GUEST);
    }
  ...
  public synchronized static User getRegisteredUser()
  throws WdkModelException, WdkUserException {
    if (registeredUser == null) {
      ...
      if (registeredUser == null) {
        ...
        registeredUser = userFactory.createUser(REGISTERED_USER_EMAIL, null,
          null, null);
        userFactory.changePassword(registeredUser.getUserId(),
  ...
  public static Question getDatasetParamQuestion() throws WdkModelException {
    if (datasetQuestions == null)
      loadQuestions();
    return datasetQuestions.get(random.nextInt(datasetQuestions.size()));
  }

  public static RunnableObj<Step> createNormalStep(User user)
  throws WdkModelException {
    Question question = getNormalQuestion();
    List<ParamValuesSet> paramValueSets = ParamValuesFactory.getParamValuesSets(
      user, question.getQuery());
    ParamValuesSet paramValueSet = paramValueSets.get(
      random.nextInt(paramValueSets.size()));
  ...
  private static void loadQuestions() throws WdkModelException {
    WdkModel wdkModel = getModel();
    var allNormalQuestions = new HashMap<String, List<Question>>();
    var allAnswerQuestions = new HashMap<String, List<Question>>();
    var allDatasetQuestions = new HashMap<String, List<Question>>();

    for (QuestionSet questionSet : wdkModel.getAllQuestionSets()) {
      for (Question question : questionSet.getQuestions()) {
        String rcName = question.getRecordClass().getFullName();

        if (!(question.getQuery() instanceof SqlQuery))
          continue;

        if (!allAnswerQuestions.containsKey(rcName)) {
          allAnswerQuestions.put(rcName, new ArrayList<>());
          allDatasetQuestions.put(rcName, new ArrayList<>());
          allNormalQuestions.put(rcName, new ArrayList<>());
        }
        ...
        if (!hasAnswerParam && !hasDatasetParam && !question.getQuery()
          .getDoNotTest())
